...
function calculateSellPrice(
  ...
  return {
    ...
    interestAmount: interestAmount.toLocaleString("en-US", {
...
export async function createSaleRequest(
  ...
  try {
    ...
    const existingSaleRequest = await db
      .collection<SaleRequest>("saleRequests")
      .findOne({
        bondRequestId,
        userEmail,
        status: { $ne: "normal" },
    ...
    const { sellPrice, interestAmount, daysHeld } = calculateSellPrice(
      originalPrice,
      purchaseDate,
      annualInterestRate
    );
...
export async function updateSaleRequest(
  ...
  try {
    ...
    const updatedSaleRequest = await db
      .collection<SaleRequest>("saleRequests")
      .findOne({ id });
