...
export async function POST(request: Request) {
  try {
    ...
    console.log("Received form submission with data:", body);
    ...
    if (bondFeatures && (!nominalPrice || !unitPrice)) {
      ...
      const nominalFeature = bondFeatures.find(
        (feature: string) => feature.includes("Нэрлэсэн үнэ:")
      );
      const unitFeature = bondFeatures.find(
        (feature: string) => feature.includes("Нэгж үнэ:")
      );
